...
describe("TarjetasService business rules", () => {
  ...
  const service = new TarjetasService(repo as never);
  ...
  it("rejects inactive cards marked as in use", async () => {
    ...
    await expect(service.create({ sedeId: 2, numero: 10, color: "#3B82F6", icono: "IdCard", areaIds: [1], activo: false, enUso: true })).rejects.toMatchObject({ response: { code: "CONFLICT" }, status: 409 });
  });
  ...
  it("rejects unknown or inactive assigned areas", async () => {
    ...
    await expect(service.create({ sedeId: 2, numero: 10, color: "#3B82F6", icono: "IdCard", areaIds: [1] })).rejects.toMatchObject({ response: { code: "CONFLICT" }, status: 409 });
  ...
  it("checks card number uniqueness within its site", async () => {
    ...
    await expect(service.create({ sedeId: 2, numero: 10, color: "#3B82F6", icono: "IdCard", areaIds: [1] })).rejects.toMatchObject({ response: { code: "CONFLICT" } });
  ...
  it("blocks deletion while a card is in use", async () => {
    ...
    await expect(service.delete(1)).rejects.toMatchObject({ response: { code: "CONFLICT" }, status: 409 });

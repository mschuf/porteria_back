...
describe("AreasService business rules", () => {
  ...
  const service = new AreasService(repo as never);
  ...
  it("blocks deactivation when assigned", async () => {
    ...
    await expect(service.deactivate(1)).rejects.toMatchObject({ response: { code: "CONFLICT" }, status: 409 });
  });
  ...
  it("checks area name uniqueness within its site", async () => {
    ...
    await expect(service.create({ sedeId: 2, nombre: "Recepción" })).rejects.toMatchObject({ response: { code: "CONFLICT" } });

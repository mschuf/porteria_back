...
export class AreasService {
  ...
  private async required(user: AuthenticatedUser, id: number) { const row = await this.repo.findById(id); if (!row) throw new BusinessException({ message: `Area ${id} no encontrada`, code: API_ERROR_CODE.NOT_FOUND, status: HttpStatus.NOT_FOUND }); await this.access.assertSede(user, Number(row.sede_id)); return row; }
